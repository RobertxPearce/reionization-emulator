...
class FitConfig:
    ...
    early_stopping_patience: Optional stop if validation loss doesn't improve for this many epochs
...
def train_one_epoch(model, loader, optimizer, loss_fn, device, gradient_clipping=None) -> float:
    """
...
def fit(model, train_loader, val_loader, optimizer, loss_fn, config: FitConfig) -> Dict[str, list]:
    """
    Train model for many epochs with optional early stopping. Calls train_one_epoch() and evaluate().
    ...
    optimizer: Optimizer to use
    loss_fn: Loss function
    config: Configuration data for epochs, device, early_stopping_patience, and gradient_clipping
    ...
    for epoch in range(1, config.epochs + 1):
        # Train one epoch
        train_loss = train_one_epoch(model, train_loader, optimizer, loss_fn, device, gradient_clipping=config.gradient_clipping)
        # Validate the epoch
